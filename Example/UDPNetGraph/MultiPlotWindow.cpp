...
void CMultiPlotWindow::SetXY(const int plotIndex, const float y, const int graphIndex)
{
	...
	m_plotWindows[ plotIndex].wnd->SetPlotXY(0, y, plotIndex);
}
...
// 	m_UpdateButton.GetClientRect(btnR);
	...
	if (plotSize > 0)
	{
		...
		const float plotH = (float)totalHeight / (float)plotSize;

		float plotT = 0;
		for each (auto &it in m_plotWindows)
		{
			if (it.wnd)
				it.wnd->MoveWindow(CRect(0, plotT, cr.Width(), (int)(plotT + plotH)));
			plotT += plotH;
		}
